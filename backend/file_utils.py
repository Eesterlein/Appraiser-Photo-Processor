...
def generate_unresolved_filename(original_name: str, date_str: str) -> str:
    """
    Generate filename for unresolved appraiser images.

    Format: UNRESOLVED - {original stem} - {DATE}.JPG
    """
    stem = Path(original_name).stem
    return f"UNRESOLVED - {stem} - {date_str}.JPG"
...
def copy_and_rename_image(
    ...
    try:
        ...
        # Note: use exif=b""/icc_profile=b"", not None — Pillow >=10 raises
        # "object of type 'NoneType' has no len()" when passed None here.
        # Empty bytes still strip EXIF/ICC, keeping the RealWare-compatible,
        # metadata-free output intent. Matches the fix applied to the other
        # save() calls in this file (_convert_heic_to_jpeg, convert_to_jpeg).
        img.save(
            full_path,
            format='JPEG',
            quality=95,
